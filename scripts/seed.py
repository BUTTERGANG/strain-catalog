    ...
    async with async_session() as db:
        ...
        if count > 0:
            ...
            if not json_path.exists():
                wikileaf_path = DATA_DIR / "wikileaf_strains.csv"
                if wikileaf_path.exists():
                    print("[seed] cannabis.json not found, importing included WikiLeaf data")
                    from scripts.import_wikileaf import main as import_wikileaf
                    await import_wikileaf()
                else:
                    print("[seed] no strain dataset found, skipping")
            else:
